...
public class ScreenScraperComponentImpl implements ScreenScraperComponent {
    ...
    public UserInfo getUserInfo(ApiCredentials credentials) throws Exception {
        ...
        return UserInfo.builder()
                ...
                .lastVisitDate(userNode.path("datedernierevisite").asText())
                .favRegion(userNode.path("favregion").asText())
                .build();
    }
        ...
        if (regionsNode.isArray()) {
            for (JsonNode node : regionsNode) {
                regions.add(Region.builder()
                        .id(node.path("id").asInt())
                        .shortName(node.path("nomcourt").asText())
                        .nameDe(node.path("nom_de").asText())
                        .nameEn(node.path("nom_en").asText())
                        .nameEs(node.path("nom_es").asText())
                        .nameFr(node.path("nom_fr").asText())
                        .nameIt(node.path("nom_it").asText())
                        .namePt(node.path("nom_pt").asText())
                        .parent(node.path("parent").asInt())
        ...
        if (mediasNode.isArray()) {
            for (JsonNode node : mediasNode) {
                mediaList.add(SystemMediaInfo.builder()
                        ...
                        .shortName(node.path("nomcourt").asText())
                        .name(node.path("nom").asText())
                        .category(node.path("categorie").asText())
                        .platformTypes(node.path("platformtypes").asText())
                        .platforms(node.path("platforms").asText())
                        .type(node.path("type").asText())
                        .fileFormat(node.path("format").asText())
                        .fileFormat2(node.path("format2").asText())
                        ...
                        .extraInfosTxt(node.path("extrainfostxt").asText())
                        .build());
            }
        }
        return mediaList;
        ...
        if (systemsNode.isArray()) {
            for (JsonNode node : systemsNode) {
                systems.add(GameSystem.builder()
                        ...
                        .extensions(node.path("extensions").asText())
                        .company(node.path("compagnie").asText())
                        .type(node.path("type").asText())
                        .startDate(node.path("datedebut").asText())
                        .endDate(node.path("datefin").asText())
                        .romType(node.path("romtype").asText())
                        .supportType(node.path("supporttype").asText())
    ...
    private Map<String, String> parseMedias(JsonNode mediasNode) {
        ...
        if (mediasNode != null && !mediasNode.isMissingNode()) {
            mediasNode.properties().forEach(entry -> 
                medias.put(entry.getKey(), entry.getValue().asText())
            );
        }
        return medias;
    }
    ...
    private Game parseGame(JsonNode node) {
        ...
        Game.GameBuilder builder = Game.builder()
                ...
                .editor(node.path("editeur").asText())
                .developer(node.path("developpeur").asText())
                .players(node.path("joueurs").asText())
                .score(node.path("note").asDouble())
                .topStaff(node.path("topstaff").asInt())
                .rotation(node.path("rotation").asText())
        ...
        if (!systemNode.isMissingNode()) {
            ...
            systemNames.put("nom", systemNode.path("nom").asText());
    ...
    private Rom parseRom(JsonNode node) {
        ...
        return Rom.builder()
                ...
                .romFileName(node.path("romfilename").asText())
                .romSerial(node.path("romserial").asText())
                .romRegions(node.path("romregions").asText())
                .romLangues(node.path("romlangues").asText())
                .romType(node.path("romtype").asText())
                .romSupportType(node.path("romsupporttype").asText())
                .romSize(node.path("romsize").asLong())
                .romCrc(node.path("romcrc").asText())
                .romMd5(node.path("rommd5").asText())
                .romSha1(node.path("romsha1").asText())
                ...
                .players(node.path("joueurs").asText())
                .dates(parseNames(node.path("dates")))
                .publisher(node.path("editeur").asText())
                .developer(node.path("developpeur").asText())
                .synopsis(parseNames(node.path("synopsis")))
                .build();
    }
